...
// // ฟังก์ชันเช็คว่าเวลากด "ลงเวลางาน" ซ้ำภายใน 15 นาทีหรือไม่
// func isRecentWorktimeAction(userID string) bool {
// 	lastActionTime, exists := userLastWorktimeAction[userID]
// 	if !exists {
// 		return false
// 	}
// 	return time.Since(lastActionTime) < 15*time.Minute
// }
...
// 	// ตรวจสอบว่าผู้ใช้กดลงเวลางานซ้ำภายใน 15 นาทีหรือไม่
// 	if isRecentWorktimeAction(userID) {
// 		// ส่งข้อความถามยืนยัน
// 		confirmMessage := linebot.NewTemplateMessage(
// 			"คุณเพิ่งลงเวลางานไปเมื่อไม่นานนี้ ต้องการดำเนินการต่อหรือไม่?",
// 			linebot.NewConfirmTemplate(
// 				"คุณเพิ่งลงเวลางานไปเมื่อไม่นานนี้ ต้องการดำเนินการต่อหรือไม่?",
// 				linebot.NewMessageAction("ยืนยัน", "ยืนยันลงเวลางาน"),
// 				linebot.NewMessageAction("ยกเลิก", "ยกเลิก"),
// 			),
// 		)

// 		if _, err := bot.ReplyMessage(event.ReplyToken, confirmMessage).Do(); err != nil {
...
// 	// ✅ ลองใช้ ReplyMessage ก่อน
// 	if _, err := bot.ReplyMessage(event.ReplyToken, flexMessage).Do(); err != nil {
// 		log.Println("❌ Error sending Flex Message UpdateWorktimeUI with ReplyMessage:", err)

// 		// ✅ ถ้า ReplyMessage ล้มเหลว ให้ลองใช้ PushMessage แทน
// 		if _, pushErr := bot.PushMessage(event.Source.UserID, flexMessage).Do(); pushErr != nil {
// 			log.Println("❌ Error sending message with PushMessage:", pushErr)
// 			sendReply(bot, event.ReplyToken, "เกิดข้อผิดพลาดในการส่งข้อความ กรุณาลองใหม่.")
// 		} else {
// 			log.Println("✅ Successfully sent with PushMessage.")
// 		}
// 	} else {
// 		log.Println("✅ Successfully sent with ReplyMessage.")
// 	}
// }
